...
module.exports = class RTCSession extends EventEmitter
{
  ...
  constructor(ua)
  {
    ...
    this._isMobileDevice = /iPhone|iPad|iPod/i.test(navigator.userAgent);
    this._id = null;
    this._ua = ua;
  ...
  answer(options = {})
  {
    ...
    if (this._isMobileDevice)
    {
      debug(`isLateSDP ${this._late_sdp}`);

      Promise.resolve()
        .then(() => 
        {
          const desc = options.localSDP;
          ...
          {
            request.reply(500);
            throw new Error(`${options.mobileWebRTCError} failed`);
          }

          const type = (! this._late_sdp) ? 'answer' : 'offer';
          
          this._createdMobileLocalDescription(type, options.sdp);
  ...
  */
  ...
  _createdMobileLocalDescription(type, sdp)
  {
    debug('createdMobileLocalDescription()');

    this._rtcReady = false;

    const e = { originator: 'local', type: type, sdp: sdp };
   ...
   */
  _MobileWebRTCDelegate(sender)
  {
    debug(`Mobile Delegate sender: ${sender}`);
    ...
    debug('emit "sending" [request:%o] from _MobileWebRTCDelegate', this._request);
    ...
    switch (true)
    {
      ...
      case /^1[0-9]{2}$/.test(response.status_code):
      {
        ...
        if (this._isMobileDevice)
        {
          ...
          const data = { originator: 'remote', type: 'answer', sdp: response.body, msg: response };
      ...
      case /^2[0-9]{2}$/.test(response.status_code):
      {
        ...
        if (this._isMobileDevice)
        {
          ...
          const data = { originator: 'remote', type: 'answer', sdp: e.sdp, msg: responseString };

          this.emit('MobileRTCEvent', 'PRACK', data);

        }
        else
        {
          const answer = new RTCSessionDescription({ originator: 'remote', type: 'answer', sdp: e.sdp, msg: response });
